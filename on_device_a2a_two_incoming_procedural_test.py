...
post1 = sim.Population(n_neurons, sim.IF_curr_exp, params,
                      label='post1')
post1.record('spikes')
...
proj = sim.Projection(pre, post, conn, synapse_type=syn, use_procedural=procedural)

syn = sim.StaticSynapse(weight=5./n_neurons, delay=1)#rand_dist)
proj = sim.Projection(pre, post1, conn, synapse_type=syn, use_procedural=procedural)
